...
void Tests::testServiceAdmin() {
    ...
    ServiceAdmin serviceAdmin{repoShelterDogs, "TestShelterDogs.txt"};

    assert(serviceAdmin.getDogs().empty());
    ...
    serviceAdmin.addDog("Rex", "German Shepherd", 3,
                        "https://en.wikipedia.org/wiki/File:German_Shepherd_-_DSC_0346_(10096362833).jpg");
    assert(serviceAdmin.getDogs().size() == 1);
...
void Tests::testRepoAdoptedDogs(){
    // testing constructor
    RepoAdoptedDogs* repoAdoptedDogs = new RepoAdoptedDogsCSV;
    assert(repoAdoptedDogs->getDogs().empty());
    ...
    repoAdoptedDogs->addDog("Rex", "German Shepherd", 3,
                           "https://en.wikipedia.org/wiki/File:German_Shepherd_-_DSC_0346_(10096362833).jpg");
    assert(repoAdoptedDogs->getDogs().size() == 1);
    ...
    assert(repoAdoptedDogs->getDogs().size() == 2);
...
void Tests::testServiceUser(){
    // testing constructor
    RepoShelterDogs repoShelterDogs;
    auto *repoAdoptedDogs = new RepoAdoptedDogsCSV;
    ServiceUser serviceUser(repoShelterDogs, repoAdoptedDogs, "TestShelterDogs.txt", "TestAdoptedDogs.txt");
    assert(serviceUser.getDogsShelter().empty());
    assert(serviceUser.getDogsAdopted().empty());
    ...
    assert(serviceUser.getDogsShelter().empty());
    assert(serviceUser.getDogsAdopted().size() == 1);
    ...
    std::vector<Dog> dogsFiltered = serviceUser.getDogsFiltered("German Shepherd", 10);
    assert(dogsFiltered.size() == 3);
