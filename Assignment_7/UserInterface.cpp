...
void UserInterface::chooseMode() {
    ...
    while (choice != 'x') {
        ...
        switch (choice) {
            ...
            case '2': /// user mode
            {
                if(serviceAdmin.getDogs().empty() && serviceUser.getDogsAdopted().empty()) {
                    try {
                        this->serviceAdmin.readShelterDogFromFile();
                    } catch (exception &exception) {
                        cout << exception.what() << endl;
    ...
    while (choice != 'x') {
        ...
        switch (choice) {
            case '1': /// add a dog
            {
                ...
                cin >> newDog;
                try {
                    this->serviceAdmin.addDog(newDog);
            ...
            {
                for (int i = 0; i < this->serviceAdmin.getDogs().size(); i++) {
                    cout << "-----> Dog " << i + 1 << " <-----" << endl;
                    cout << "Name: " << this->serviceAdmin.getDogs()[i];
                }
            }
                break;
            case 'x': /// exit
    ...
    while (choice != 'x') {
        ...
        switch (choice) {
            ...
            case '4': /// see what dogs you've adopted already
            {
                ...
                for (int i = 0; i < this->serviceUser.getDogsAdopted().size(); i++) {
                    cout << "-----> Dog " << i + 1 << "<-----" << endl;
            ...
            case '5': /// see the dogs of a breed and age < number
            {
                ...
                age = stoi(stringAge);
                std::vector<Dog> filteredDogs = this->serviceUser.getDogsFiltered(breed, age);
                if (filteredDogs.empty()) {
                    cout << "There are no Dogs with this breed and younger than " << age << endl;
                    break;
                }
                int index = 0;
                while (index < filteredDogs.size()) {
                    ...
                    while (choice2 != 'x') {
                        ...
                        switch (choice2) {
                            case '2': /// adopt the current dog
                            {
                                ...
                                // removing the dog from the filtered list
                                filteredDogs.erase(filteredDogs.begin() + index);
                                cout << "Dog adopted successfully!" << endl;
                                if (filteredDogs.empty()) {
                                    cout << "There are no more dogs left!" << endl;
                                    goto outOfThisFunctionality;
                                }
                                index = (index + 1) % filteredDogs.size();
                                choice2 = 'x';
                            }
                                break;
                            case '3': /// go to the next dog
...
void UserInterface::having10DogsAtStartUp(){
    ...
    if(this->serviceAdmin.getDogs().empty())
    {
        ...
        serviceAdmin.saveShelterDogToFile();
    }
